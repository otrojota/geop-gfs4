...
class DescargaPronostico {
    ...
    async importa_old(fileName) {        
        try {
            ...
            for (let i=0; i<info.bands.length; i++) {
                ...
                while(j <variables.length && !agrego) {
                    ...
                    if (idx >= 0) {
                        ...
                        tgtMetadata.variables[variable.codigo + "-" + idx] = {
            ...
            let outMetadata = await gdal.info(outPath + "/tmp_" + outFileName, true);
            //console.log("outMetadata", outMetadata);
            Object.keys(tgtMetadata.variables).forEach(codigo => {
                let variable = tgtMetadata.variables[codigo];
                let banda = variable.banda;
                let m = outMetadata.bands.find(m => m.band == banda);
                variable.min = m.computedMin;
                variable.max = m.computedMax;
                if (m.noDataValue !== undefined) variable.noDataValue = m.noDataValue;
            });
