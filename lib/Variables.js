...
class Variables {
    ...
    constructor() {
        this.cache = {}; // YYYYMMDDHH:{timestamp:999999, metadata:{...}}
    ...
    checkCache() {
        if (Object.keys(this.cache).length > MAX_CACHE) {
            let list = Object.keys(this.cache).reduce(list, key => {
                let c = this.cache[key];
                list.push({key:key, timestamp:c.timestamp, metadata:c.metadata});
                return list;
            }, []).sort((c1, c2) => (c1.timestamp - c2.timestamp));
            while (list.length > CLEAN_CACHE_TO) {
                list.splice(0,1);
            }
            this.cache = list.reduce((acum, c) => {
                acum[c.key] = {timestamp:c.timestamp, metadata:c.metadata};
                return acum;
            }, {})
        }
    }
    getMetadata(timestamp) {
        return new Promise((resolve, reject) => {
            ...
            let c = this.cache[dt.format("YYYYMMDDHH")];
            if (c !== undefined) {
            ...
            fs.readFile(path, (err, data) => {
                ...
                this.cache[dt.format("YYYYMMDDHH")] = {timestamp:new Date().getTime(), metadata:metadata};
                this.checkCache();
                resolve(metadata);
            });
        });
    ...
    setMetadata(timestamp, metadata) {
        return new Promise((resolve, reject) => {
            ...
            fs.writeFile(path, JSON.stringify(metadata), err => {
                ...
                this.cache[dt.format("YYYYMMDDHH")] = {
    ...
    async findMetadata(timestamp, dsCode, levelIndex) {
        let dsCodeSearch = dsCode;
        if (dsCode == "VIENTO_10m") dsCodeSearch = "UGRD_10M";
        else if (dsCode == "VIENTO") dsCodeSearch = "UGRD_HP";
        else dsCodeSearch = dsCode;
        let varCode = dsCodeSearch + "-" + levelIndex;
        ...
        while (intentos < 8) {
            ...
            if (m && m.variables[varCode]) return m;
            if (intentos > 0) {
                t = t0 + 3*intentos;
